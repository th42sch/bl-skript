...
\begin{enumerate}
  ...
  \item \emph{lokaler Rollenwertvergleich:}
    %
    \[\text{KorrekterStudent}\equiv \text{Student}\sqcup
    (\text{hört}\circ\text{angebotenVon}\sqsubseteq
  \text{eingeschriebenAn})\]
    %
    Umgangssprachlich: ``korrekte Studierende sind Studierende, die an
    allen Einrichtungen, an denen sie einen Kurs sind, auch
    eingeschrieben sind''
...
\begin{align*}
  %
  (\text{hört}\circ\text{angebotenVon})^\Imc(\text{hanna}) &=
  \{\text{uni$\_$hb}\} \\
  %
  (\text{hört}\circ\text{angebotenVon})^\Imc(\text{klaus}) &=
  \{\text{uni$\_$hb},\text{hs$\_$hb}\} \\
  %
  (\text{hört}\circ\text{angebotenVon})^\Imc(\text{BL}) &= \emptyset \\
  %
  & ~\vdots \\
  %
...
\begin{align*}
  ...
  %
  &\tau(0,1) = t_1, \quad\tau(1,1)=t_3, \quad\ldots, \\
  %
...
\begin{center}
  ...
  \node (p1)  [label=below right:{$A_{t_1}$}] {$(0,0)$}; 
  \node (p2)  [label=below right:{$A_{t_2}$},right=2cm of p1]  {$(1,0)$};
  \node (p3)  [label=below right:{$A_{t_3}$},right=2cm of p2]  {$(2,0)$};
  \node (p4)  [draw=none,right=2cm of p3]  {$\ldots$};
    
  \node (q1)  [label=below right:{$A_{t_3}$},above=2cm of p1] {$(0,1)$}; 
  \node (q2)  [label=below right:{$A_{t_1}$},right=2cm of q1]  {$(1,1)$};
  \node (q3)  [label=below right:{$A_{t_2}$},right=2cm of q2]  {$(2,1)$};
