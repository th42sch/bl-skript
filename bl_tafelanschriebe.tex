...
Da $C_0 \in \Lmc(v_{\textsf{ini}})$ in $B_{\textsf{ini}}$,
gilt auch $C_0 \in \Lmc(v_{\textsf{ini}})$ in $B$,
  ...
  \item[Induktionsschritt.]
    ...
    \begin{description}
      \item[{\boldmath $C = \lnot A$}]
        ...
        Da der I-Baum $B$ keinen offensichtlichen Widerspruch enthält,
        folgt aus $\lnot A \in \Lmc(v)$,
        dass $A \notin \Lmc(v)$.
        Nach Definition von \Imc gilt dann aber $v \notin A^\Imc$,
        also $v \in (\lnot A)^\Imc$.
      \item[{\boldmath $C = D \sqcap E$.}]
        Wenn $C \in \Lmc(v)$ in $B$,
        dann auch $D \in \Lmc(v)$
        und $E \in \Lmc(v)$,
        da $B$ vollständig ist und somit insbesondere die
...
Das folgende Bild zeigt einen I-Baum $B$ (links)
und eine Interpretation \Imc, die $B$ realisiert, (rechts)
sowie die entsprechende Funktion $\pi$ (gestrichelte Pfeile).
%
\begin{center}
  \begin{tikzpicture}[%
    >=Latex,baseline=.2pt,
    ...
    \node [above=5mm of leps] {I-Baum $B$};
...
\end{center}
%
Man beachte, dass die Knoten von $B$ (links) mit Mengen von Konzepten beschriftet sind,
