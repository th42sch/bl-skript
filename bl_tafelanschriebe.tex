...
\begin{description}
  ...
  \item[{\boldmath "`$\Leftarrow$"'}] 
    ...
    \begin{itemize}
      \item
        eine Simulation $\rho_1$ von $\Imc_D$ nach $\Imc_C$ mit $\rho_1(d_W) = d_W$\quad und
      \item
        eine Simulation $\rho_2$ von $\Imc_C$ nach $\Imc$ mit $\rho_2(d_W) = d$.
    \end{itemize}
    %
    Die Komposition $\rho_1 \circ \rho_2$ ist eine Simulation von $\Imc_D$
    nach \Imc mit $(\rho_1 \circ \rho_2)(d_W) = d$;
...
\begin{center}
  ...
  \node (da3)  [below right=2cm of da1,label=below:{$A_3$}]  {$d_{A_3}$};
  \node (da2)  [right=4cm of da1, label=above:{$A_1,A_2,A_3$}] {$d_{A_2}$};
   
  \draw[->] (da1) edge node[draw=none,above]
  {$s$} (da2);
  \draw[->] (da1) edge node[draw=none,pos=.5, left=1mm]
  {$r$} (da3);
  \draw[->] (da2) edge node[draw=none,pos=.5,right=1mm]
  {$r$} (da3);
