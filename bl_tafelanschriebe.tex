...
\section*{5.1~ Beispiel für Typen}
...
%
\[\text{sub}(C_0,\Tmc) = \{A,C_\Tmc,\exists r.\exists r.A,\exists r.A,
\forall r.A',A',\neg A\sqcup \neg A',\neg A,\neg A'\}\]
%
...
\section*{5.2~ Korrektheitsbeweis}
...
%
\[\text{sub}(C_0,\Tmc)=\{A,C_\Tmc,\forall r.\exists r.A,\exists
r.A,\neg A\sqcup\exists r.A,\neg A\}.\]
%
...
Beweis der \textbf{Behauptung}: Für alle $C\in \text{sub}(C_0,\Tmc)$
...
\begin{itemize}
  ...
  \item Aus Bedingung 4 für Typen folgt dass $C_\Tmc\in t$, für jedes
    ...
    für alle $t\in \Gamma_i=\Delta^\Imc$, und daher insbesondere
    $\Imc\models\Tmc$.

\end{itemize}

\section*{5.3~ Vollständigkeitsbeweis}

Wir beweisen die \textbf{Behauptung}, dass $\Gamma\subseteq \Gamma_i$,
für alle $i\leq k$ durch Induktion. 

\medskip
\textbf{Induktionsanfang} $i=0$. Alle
(semantischen) Typen in $\Gamma$ erfüllen die Bedingungen~1-4 von
syntaktischen Typen, daher ist $\Gamma\subseteq \Gamma_0$.

\smallskip
\textbf{Induktionsschritt} Sei $i>0$ und $t\in \Gamma\subseteq
...
$\Rightarrow$\quad $S\subseteq \text{tp}_\Imc(e)$ und
$t'=\text{tp}_\Imc(e)\in \Gamma\subseteq\Gamma_{i-1}$. 
